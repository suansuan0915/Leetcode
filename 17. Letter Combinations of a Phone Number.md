...
Why TC:
- a number corresponding to up to 4 letters, a digit has n chars, so total combinations can be 4**n.
- for each comination, join path costs n.
- So, in total: `4**n * n`.
...
class Solution:
    ...
    def backtrack(self, index, path, ans):
        ...
        # No need "for i in range(idx, len(self.digits)):" here, becoz starting idx 0 will always return full ans, but if starting with later idx, never reach a valid result.
