...
- Method 2:\
  ...
  Cannot add `if p == q:` before compare `l and r`:
  - `if p == q:` is only True when p and q are literally the same node object in memory, not when they merely have the same structure/values.
